...
int is_recv_custom_packet(struct sk_buff *skb){
    u8 buff[16];
    if(!skb) return 1;

    skb_copy_bits(skb, 0, buff, 16);
    //printk(KERN_DEBUG "DANXTEST :%02x %02x %02x %02x %02x %02x\n",
    //                            buff[0], buff[1], buff[2], buff[3], buff[4], buff[5] );
    if (buff[0] == 0x00 && buff[1] == 0xFF) {
        printk(KERN_DEBUG "DANX Recv Custom packet\n");
        return 0;
    }
    return 1;
}
				    ...
				    struct net_device *netdev)
{
	...
	if (unlikely(skb->no_fcs))
        ...
        if (is_custom_packet(skb) == 0) {
        	...
        	printk(KERN_DEBUG "DANXTRANS: %llu %d %s:%s:%d, Len : %d \n"
        	       ,now, tx_ring->next_to_use, __FILE__ , __func__ ,__LINE__, skb->len);
	}
...
static irqreturn_t e1000_intr(int irq, void *data)
{
	...
	if (likely(napi_schedule_prep(&adapter->napi))) {
        ...
        unsigned long long now = ktime_to_ns(ktime_get_real());
        recv_time = now;
        //printk(KERN_DEBUG "DANXRING: %llu %s:%s:%d\n"
        //       ,now, __FILE__ , __func__ ,__LINE__);
	...
	while (rx_desc->status & E1000_RXD_STAT_DD) {
		...
		if (unlikely(!(status & E1000_RXD_STAT_EOP)))
			adapter->discarding = true;
                ...
                /*
                if (is_recv_custom_packet(skb) == 0) {
                        unsigned long long now = ktime_to_ns(ktime_get_real());
                        unsigned long long tstamp = ktime_to_ns(skb->tstamp);
                        memcpy(skb_put(skb,8), &recv_time, 8);
                        printk(KERN_DEBUG "DANXRECV: %s:%s:%d, Len : %d %llu %llu\n"
                           , __FILE__ , __func__ ,__LINE__, skb->len, now, recv_time);
	        }*/
